...
if __name__ == "__main__":
	...
	if il_vs_th == 0:
		...
		audio_coeff = signal.firwin(audio_taps, audio_Fc / (rf_Fs / (rf_decim * 2)), window='hamming')
	else:
	...
	while (block_count + 1) * block_size < len(iq_data):
		...
		if il_vs_th == 0:
			...
			fm_demod, state_I, state_Q = fmDemodArctan(i_ds, q_ds, state_I, state_Q)
		else:
		...
		audio_filter, state_audio = signal.lfilter(audio_coeff, 1.0, fm_demod, zi=state_audio)
		audio_block = audio_filter[::audio_decim]
		...
		if block_count >= 10 and block_count < 12:
			...
			'''
			ax1.clear()
			fmPlotPSD(ax1, audio_filter, (rf_Fs / rf_decim) / 1e3, subfig_height[1], 'Filtered Mono Audio')
			ax2.clear()
